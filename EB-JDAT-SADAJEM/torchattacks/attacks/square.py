...
class Square(Attack):
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == "Linf":
                x_best = torch.clamp(
                    x + self.eps * self.random_choice([x.shape[0], c, 1, w]), -1.0, 1.0
                )
                ...
                x_best = torch.clamp(
                    x + self.normalize_delta(delta_init) * self.eps, -1.0, 1.0
                )
                margin_min, loss_min = self.margin_and_loss(x_best, y)
                n_queries = torch.ones(x.shape[0]).to(self.device)
                s_init = int(math.sqrt(self.p_init * n_features / c))
                ...
                for i_iter in range(self.n_queries):
                    ...
                    x_new = torch.clamp(
                        x_curr + self.normalize_delta(delta_curr) * self.eps, -1.0, 1.0
                    )
                    x_new = self.check_shape(x_new)
