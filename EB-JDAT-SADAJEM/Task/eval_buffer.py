...
def cond_is_fid(f, new_buffer, args, device, ratio=0.1, eval='all'):
    ...
    all_y = t.cat(all_y, 0)
    probs = t.cat(probs, 0)
    # print(f'new_buffer device: {new_buffer.device}')
    # print(f'all_y device: {all_y.device}')
...
def eval_is_fid(replay_buffer, device, args, eval='all'):
    # from Task.inception import get_inception_score
    # from Task.fid import calculate_fid_given_paths
