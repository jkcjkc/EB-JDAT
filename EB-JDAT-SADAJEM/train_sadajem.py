...
def normalize_tensor(in_feat, eps=1e-10):
    norm_factor = torch.sqrt(torch.sum(in_feat ** 2))  
...
def main(arg):
    ...
    if arg.px > 0 or args.load_path is None:
        if arg.init == 'gm':
    ...
    for epoch in range(arg.n_epochs):
        ...
        for i, (x_lab, y_lab, idx) in tqdm(enumerate(dload_train_labeled)):
            ...
            if (arg.p_x_xadv_weight > 0) or (arg.p_y_given_xadv_weight > 0):  # maximize log p(x, xhead)
                
                
                xadv_lab = sampleXadv(f, x=x_lab, y=y_lab, in_steps=arg.in_steps , args=arg)
                

                # print(xadv_lab.shape)
                fp, fq = f(x_p_d).mean(), f(xadv_lab).mean()
                l_p_x_xadv = -(fp - fq)
               
                if arg.p_x_xadv_weight > 0:
                    loss += args.p_x_xadv_weight * l_p_x_xadv
                if arg.p_y_given_xadv_weight > 0:  # maximize log p(y | xadv)
                    ...
                    loss += args.p_y_given_xadv_weight * l_p_y_given_xadv
            optim.zero_grad()
            loss.backward()
            ...
            if (arg.p_x_xadv_weight > 0) or (arg.p_y_given_xadv_weight > 0):  # maximize log p(x, xhead)
                ...
                l_p_x_xadv = -(fp - fq)
                if cur_iter % args.print_every == 0:
                    print('P(xadv|x) | {}:{:>d} f(x_p_d)={:>9.4f} f(x_q_adv)={:>9.4f} d={:>9.4f}'.format(epoch, i, fp, fq, fp - fq))
                if arg.p_x_xadv_weight > 0:
        ...
        if epoch % arg.ckpt_every == 0  and epoch >= 30:
...
if __name__ == "__main__":
    ...
    # EBM specific
    parser.add_argument("--n_steps", type=int, default=5, help="number of steps of SGLD per iteration, 20 works for PCD")
    parser.add_argument("--in_steps", type=int, default=5, help="number of steps of PGD")
